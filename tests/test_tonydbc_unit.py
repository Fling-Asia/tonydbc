...
class TestTonyDBCOnlineOnly:
    ...
    def test_init_with_all_parameters(self):
        ...
        assert db.interact_after_error == True
    ...
    def test_make_connection_retry_on_failure(self, mock_connect, tonydbc_instance):
        ...
        mock_connect.side_effect = [
            mariadb.Error("Connection failed"),
            mariadb.Error("Connection failed"),
            mock_conn
        ]
    ...
    def test_set_timezone_with_parameter(self, tonydbc_instance):
        ...
        with patch.dict(os.environ, {'DEFAULT_TIMEZONE': 'UTC'}):
    ...
    def test_set_timezone_with_default(self, tonydbc_instance):
        ...
        with patch.dict(os.environ, {'DEFAULT_TIMEZONE': 'Europe/London'}):
            db.set_timezone()
    ...
    def test_start_temp_conn(self, tonydbc_instance):
        ...
        assert db.using_temp_conn == True
    ...
    def test_start_temp_conn_already_using_temp_raises_error(self, tonydbc_instance):
        ...
        with pytest.raises(AssertionError, match="already using your temporary connection"):
            db.start_temp_conn()
    ...
    def test_close_temp_conn(self, tonydbc_instance):
        ...
        assert db.using_temp_conn == False
    ...
    def test_use_changes_database(self, tonydbc_instance):
        ...
        with patch.object(db, '__exit__') as mock_exit, \
    ...
    def test_update_table(self, tonydbc_instance):
        ...
        with patch.object(db, 'get_column_datatypes', return_value={"name": str}), \
    ...
    def test_update_blob_file_too_large(self, tonydbc_instance):
        ...
        with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
            ...
            with pytest.raises(AssertionError, match="File size"):
                db.update_blob("test_table", "blob_col", 1, tmp_file.name, max_size_MB=16)
    ...
    def test_production_databases_empty_list(self, tonydbc_instance):
        ...
        with patch('tonydbc.tonydbc.get_env_list', side_effect=KeyError("PRODUCTION_DATABASES")):
            result = db.production_databases
    ...
    def test_drop_database_success(self, tonydbc_instance):
        ...
        with patch.object(db, 'production_databases', []), \
             patch.object(db, 'execute') as mock_execute:
    ...
    def test_post_data(self, tonydbc_instance):
        ...
        result = db.post_data("INSERT INTO test VALUES (1)")
    ...
    def test_post_datalist(self, tonydbc_instance):
        ...
        result = db.post_datalist("INSERT INTO test VALUES (?, ?)", data)
        
        mock_cursor.executemany.assert_called_once_with("INSERT INTO test VALUES (?, ?)", data)
    ...
    def test_execute_with_values(self, tonydbc_instance):
        ...
        mock_cursor.execute.assert_called_once_with("INSERT INTO test VALUES (?)", (123,))
    ...
    def test_execute_script_with_return_values(self, tonydbc_instance):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.sql', delete=False) as tmp_file:
            ...
            with patch.object(db, 'get_data', return_value=[{"result": 1}]) as mock_get_data:
    ...
    def test_non_primary_keys(self, tonydbc_instance):
        ...
        with patch.object(db, 'get_column_datatypes', return_value={"id": int, "name": str, "email": str}), \
    ...
    def test_query_table_with_custom_query(self, tonydbc_instance):
        ...
        with patch.object(db, 'read_dataframe_from_table', return_value=mock_df) as mock_read:
            result = db.query_table("test_table", query="SELECT id FROM test_table WHERE id = 1")
...
class TestTonyDBC:
    ...
    def test_init_sets_media_base_path(self, mock_connection):
        ...
        with patch('tonydbc.tonydbc.mariadb.connect', return_value=mock_conn), \
             patch.dict(os.environ, {'MEDIA_BASE_PATH_PRODUCTION': '/custom/path'}):
            db = TonyDBC(host="localhost", user="test", password="test", database="test")
    ...
    def test_is_online_property_getter(self, tonydbc_instance):
        ...
        assert db.is_online == True  # Default state
    ...
    def test_flush_updates_processes_queue(self, tonydbc_instance):
        ...
        db._TonyDBC__update_queue.put(("execute", {"command": "CREATE TABLE test (id INT)"}))
        db._TonyDBC__update_queue.put(("post_data", {"query": "INSERT INTO test VALUES (1)"}))
    ...
    def test_offline_read_operations_raise_error(self, tonydbc_instance):
        ...
        with pytest.raises(AssertionError, match="Cannot read from database while offline"):
    ...
    def test_offline_connection_methods_raise_error(self, tonydbc_instance):
        ...
        with pytest.raises(AssertionError, match="Cannot use database connection while offline"):
            db.start_temp_conn()
